                    ...
                    if (found) {
                        ...
                        if (distance < 6.0) {
                            if (signEmployee.loginTime != null) {
                                Toast.makeText(v.getContext(), signEmployee.name + " is already signed in", Toast.LENGTH_SHORT).show();

                            } else {
                                Date currentTime = Calendar.getInstance().getTime();
                                signEmployee.loginTime = currentTime;
                                EmployeeHelper db = new EmployeeHelper(getApplicationContext());
                                db.updateLoginTime(signEmployee.id);
